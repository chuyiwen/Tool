...
namespace 
{
char    g_CharPool[CHAR_POOL_SIZE];
wchar_t g_WCharPool[CHAR_POOL_SIZE];
...
// 将unicode转成unicode8
//-----------------------------------------------------------------------------
char const* UnicodeToUnicode8(wchar_t const* szSrc, char* szOut)
{
	if( !szOut )
		szOut = g_CharPool;
	char* szDest = szOut;
	...
	while( *szSrc )
	{
		...
		{
			*szDest++ = (uint8_t)*szSrc;
		}
		else if( *szSrc < 0x800 )	// 110xxxxx 10xxxxxx
		{
			*szDest++ = ((uint8_t)(MASK2BYTES | *szSrc >> 6));
			*szDest++ = ((uint8_t)(MASKBYTE | *szSrc & MASKBITS));
		}
		else	// 1110xxxx 10xxxxxx 10xxxxxx
		{
			*szDest++ = ((uint8_t)(MASK3BYTES | *szSrc >> 12));
			*szDest++ = ((uint8_t)(MASKBYTE | *szSrc >> 6 & MASKBITS));
			*szDest++ = ((uint8_t)(MASKBYTE | *szSrc & MASKBITS));
		}
		++szSrc;
	}
...
// 将unicode8转成unicode
//-----------------------------------------------------------------------------
wchar_t const*	Unicode8ToUnicode(char const* szSrc, wchar_t* szOut)
{
	if( !szOut )
		szOut = g_WCharPool;

	wchar_t* szDest = szOut;
...
char const* UnicodeToAnsi(wchar_t const* szSrc)
{
	g_CharPool[0] = 0;
#ifdef _WIN32
	WideCharToMultiByte(CP_ACP, 0, szSrc, -1, g_CharPool, CHAR_POOL_SIZE, NULL, NULL);
#else
	#error not implement...
#endif
	return g_CharPool;
...
wchar_t const* AnsiToUnicode(char const* szSrc)
{
	g_CharPool[0] = 0;
#ifdef _WIN32
	MultiByteToWideChar(CP_ACP, 0, szSrc, -1, g_WCharPool, CHAR_POOL_SIZE);
#else
	#error not implement...
#endif
	return g_WCharPool;
...
// 计算32位CRC,比一般算法快速很多
//-----------------------------------------------------------------------------
uint32_t Crc32(char const* lpData)
{
	...
	while( *pData )
	{
		unResult = (unResult << 8 | *pData) ^ g_Crc32Table[unResult >> 24];
		pData++;
	}
	return ~unResult;
}
...
// 计算32位CRC,比一般算法快速很多
//-----------------------------------------------------------------------------
uint32_t Crc32(wchar_t const* lpData)
{
	uint8_t* pData = (uint8_t*)lpData;
	if( !(*pData || *(pData+1)) ) 
		return (uint32_t)-1;
...
void StringToToken(std::vector<tstring>& token, TCHAR const* szString, TCHAR chSeparator/*=_T(' ')*/)
{
	...
	for(TCHAR const* pStr = szString; *pStr; pStr++)
	{
		if( *pStr == chSeparator )
		{
			if( !str.empty() )
...
void MakeDirForFile(tstring const& path)
{
    ...
    {
        _tmkdir( path.substr(0, pos).c_str() );
...
void ThrowException(TCHAR const* szText, ...)
{
	...
	throw std::exception( UnicodeToAnsi(szTextBuf) );
}
